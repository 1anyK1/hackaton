...
import queue
from crypto_protocol import ChatCrypto

MHZ = int(1e6)
TX_REPEATS = 1
TX_REPEAT_PAUSE = 0.35


# Function to transmit the message
def transmit_message(message, mySDR, crypto):
    msg = crypto.encrypt(message)
    for repeat_index in range(TX_REPEATS):
        operation_TX(mySDR, msg, False, False)
        if repeat_index < TX_REPEATS - 1:
            time.sleep(TX_REPEAT_PAUSE)


# Function to handle user input
def input_and_transmit(input_queue, exit_event, mySDR, crypto):
    while not exit_event.is_set():
        try:
            ...
            if user_input.lower() == 'q':
                ...
                break
            msg = user_input
            transmit_message(msg, mySDR, crypto)
            if crypto.mode != "none":
                print(f'sent encrypted ({crypto.label})')
        except ValueError:
...
def print_message(exit_event, mySDR, crypto):
    last_payload = None
    while not exit_event.is_set():
        receivedMessage = operation_RX(mySDR, False)
        if receivedMessage == None:
            pass
        elif receivedMessage == last_payload:
            pass
        else:
            try:
...
def encryption_menu(crypto):
    while True:
        print("++++++++++ Encryption Selection +++++++++")
        ...
        if selection == 1:
            ...
            key = input("Enter shared AES key/password: ")
            if not key:
...
def main_menu(mySDR: MyRadio):
    ...
    while True:
        ...
        if choice == 1:
            ...
            encryption_menu(crypto)
        elif choice == 5:
            if mySDR is None:
                print("Add a radio first.")
            else:
                print(f"Chat mode: {crypto.label}. Type q to quit.")
                chat(mySDR, crypto)
        elif choice == 6:
