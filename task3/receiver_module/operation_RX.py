...
def filter_frame_peaks(correlation_indices, correlation_values,
                       min_frame_distance):
    if len(correlation_indices) == 0:
        return correlation_indices, correlation_values

    filtered_indices = [correlation_indices[0]]
    filtered_values = [correlation_values[0]]
    last_real_index = np.real(correlation_indices[0])

    for index, value in zip(correlation_indices[1:], correlation_values[1:]):
        current_real_index = np.real(index)
        if current_real_index - last_real_index >= min_frame_distance:
            filtered_indices.append(index)
            filtered_values.append(value)
            last_real_index = current_real_index

    return np.array(filtered_indices), np.array(filtered_values)


def operation_RX(my_SDR: MyRadio, plot_graphs: bool):
    debug = os.environ.get("PLUTO_CHAT_DEBUG") == "1"
    ...
    correlation_indices, correlation_values = filter_frame_peaks(
        correlation_indices=correlation_indices,
        correlation_values=correlation_values,
        min_frame_distance=FRAME_LENGTH_SYMBOLS - HEADER_LENGTH)
    if plot_graphs:
        plot_spectrum(rx, Ts)
    ...
    try:
        received_message = frame_generator_RX(
            ...
            modulation_type='QAM4_2')
        if debug:
            print(f"[rx-debug] raw frame text: {received_message!r}")
        message_output = message_handler(received_message,
                                         display_outputs=False)
        if debug:
            print(f"[rx-debug] decoded message: {message_output!r}")
        return message_output
    except Exception as error:
        if debug:
            print(f"[rx-debug] receive/decode failed: {error}")
        return None
