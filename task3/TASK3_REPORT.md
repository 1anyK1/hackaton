...
Также добавлен учебный режим Snow3G-style stream cipher:

- префикс сообщения: `SNOW1:`
- ключ: 128-битный ключ, полученный из общего пароля
- nonce: 16 случайных байт на каждое сообщение
- шифрование: генерация потоковой гаммы и XOR с plaintext
- контроль целостности: HMAC-SHA256, укороченный до 16 байт
- формат пакета: `SNOW1:<payload_length>:<payload>`

Этот режим сделан в стиле Snow 3G как потоковое шифрование с регистровым состоянием и FSM-подобным обновлением. Для учебного задания он дает отдельный потоковый режим, но не является сертифицированной реализацией полного стандарта 3GPP SNOW 3G.

Также добавлен режим Caesar для демонстрации перехвата:

- префикс сообщения: `CAE1:<shift>:`
- буквы сдвигаются на выбранное значение
- после сдвига текст упаковывается в radio-safe Base64
...
Расшифровка сообщения Snow3G-style:

```bash
python3 decrypt_capture.py --cipher snow3g --key "team-password" "SNOW1:..."
```
...
- `crypto_protocol.py` - реализация AES-128-GCM, Snow3G-style и Caesar.
